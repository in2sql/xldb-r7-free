		...
		switch (message.type) {
			...
			case "onReadSheets":
				window.Asc.plugin.callCommand(function() {				
					...
					result['sheets']=[];
					result['name_document']= Api.GetFullName();
								
					sheets.forEach(element => {
						sheet = {};
						sheet['name_sheet']=element.GetName();
						sheet['table_sheet']=[];
						defnames=element.GetDefNames();
						if(defnames!=undefined && defnames.length>0){
							bFind=false;							
							defnames.forEach(defname => {								
								range=defname.GetRefersToRange();
								let nameDiap=defname.GetName();
								if(range!=undefined&&nameDiap!=undefined){
									let cell=range.GetCells(1,1);
									rangeComment=cell.GetComment();																		
									if(rangeComment!=undefined)
									{															
										commentText=rangeComment.GetText();								
										if(commentText.length>0){
											res={};
											diap={};
											resComment=JSON.parse(commentText);											
											diap['name_range']=nameDiap;
											res['query']=resComment['query'];
											res['name']=resComment['full_name'];
											res['date_time']=resComment['query_datetime'];
											diap['properties']=res;
											sheet.table_sheet.push(diap);
											bFind=true;
										}																		
									}
								}
					...
					let t=JSON.stringify(result);									
					return t;
					},false, false,function(result){											
						modal.command('onSearchRangeInWorkbook', result);						
					});					
				break;			
			case "onCancelMethod":				
    ...
    {		
		...
		$(function() {
		   ...
		   $( "#dialog-warning" ).dialog({
				modal: true,				
				width: 200,
				title:"Внимание!",//window.Asc.plugin.tr(''),
				autoOpen: false,
				buttons: {
					Ok: function() {
						$( this ).dialog( "close" );
	...
	function fillContextMenu(arr,nameMenu,id_menu){
		...
		arr.forEach(itm=>{
			if(itm.length>20)
				...
				nameItem=itm;
			strMenu="<li id='"+ nameMenu +"_"+ i +"'><a href='#"+nameMenu+"_"+i+"'>"+nameItem+"</a></li>";
